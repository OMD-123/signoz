...
func TestListQuerySelectsAllAttributeHomes(t *testing.T) {
	...
	testCases := []struct {
		...
		{"BeforeRollout_ScansAllHomes", uint64(rel - 2*day), uint64(rel - day)},
		{"AfterRollout_ScansAllHomes", uint64(rel + day), uint64(rel + 2*day)},
		{"StraddlingRollout_ScansAllHomes", uint64(rel - day), uint64(rel + day)},
	}
